...
public class JMHWars {
    ...
    public static void main(String[] args) throws RunnerException {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-sz")) {
                sz = Integer.parseInt(args[i + 1]);
            }
        }

        System.out.println("INPUT_SIZE: " + sz); 

        for (int i = 0; i <= PARTIALLY_SORTED_ARRAY_FLOATS; i++) {
            dataType = i;
            Options opt = new OptionsBuilder()
                    .include(JMHWars.class.getSimpleName())
                    ...
                    .jvmArgs("-DinputSize=" + sz, "-DdataType=" + dataType)
                    .build();
