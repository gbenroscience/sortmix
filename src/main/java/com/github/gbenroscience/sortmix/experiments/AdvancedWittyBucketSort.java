...
/**
 * Powerful bucket-sort algorithm combining memory
 * optimization with double speed optimization to
 * produce a high speed sort.
 * Sorting an array of floating points requires only an extra array 
 * of equal size and most importantly, of ints, which raises the 
 * memory efficiency by almost 50%(it handles a max array size 
 * of about 18E6 instead of about 12E6 that obtains when both are
 * arrays of floating-point numbers.)
 * Optimizing speed occurs in 2 stages.
 * <ol>
 * <li> 3 stage cache optimization, which uses a quicksort partitioning
 * stage algorithm to bring similar elements close together. 
 * What this accomplishes is to reduce cache-misses in the 
 * stages that compute the bucket sizes, an effect which is readily
 * observed in large arrays.
 * <li> Uses only 1 write in its bucket filling stage, instead of 
 * multiple writes in an earlier version and 2 writes in a more
 * advanced version.
 * 
 * </ol>
 * @author GBEMIRO
...
public class AdvancedWittyBucketSort {
  /**
     ...
     * negative and positive values and then stores the index of the last negative 
     * value in this field.
     */
       ...
       if(len>=1000000 && cacheOptimized ){
        ...
        negIndex = partitionNegativesAndPositives(sortJob, left, right);
           ...
           // printArray("bucket sizes ", bucketSizes);
             ...
             * compared to that used at the same stage in class <code>SpeedyBucketSort</code>
