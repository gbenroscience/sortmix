 ...
 *
 */
    public CustomScanner( String input, boolean includeTokensInOutput, String[] splitterTokens1, String... splitterTokens2 ) {
        this.input = input;
...
private boolean isSplittingToken( String testToken ){
    int size = splittingTokens.size();
    for(int i=0; i < size;i++){
        if(splittingTokens.get(i).equals(testToken)){
            return true;
        }
    }//end for
    return false;
 ...
 * @throws NullPointerException if no element of
 * the splittingToken array is found in the string
 * to be split.
 */
...
int longestSplittingToken = getLongestSplitterTokenLength();
 ...
 try{
   if(isSplittingToken(input.substring(i,i+j))&&i+j<=input.length()){
...
while(input.length()>0){
    ...
    if(!op.equals("")){
    split.add(op);
    input=input.substring( input.indexOf(op)+op.length()       );
...
if(opIndex==0){
    ...
    scanner.add(op);
    input=input.substring(opIndex+op.length());
}
else{
